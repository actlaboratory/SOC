...
class InstalledAppFlow(Flow):
	...
	def prepare_run_local_server(
		...
		self.local_server = wsgiref.simple_server.make_server(
			host, port, self.wsgi_app, handler_class=_WSGIRequestHandler)

		self.redirect_uri = 'http://{}:{}/'.format(
			host, self.local_server.server_port)
		auth_url, _ = self.authorization_url(**kwargs)
		...
		print(authorization_prompt_message.format(url=auth_url))
		thread=threading.Thread(target=self.localServerThread,args=(self.local_server,))
		thread.start()
		return auth_url
	...
	def run_local_server(self):
		...
		Returns:
			google.oauth2.credentials.Credentials: The OAuth 2.0 credentials
				for the user.
		"""
		if self.wsgi_app.last_request_uri==None:return None
		# Note: using https here because oauthlib is very picky that
		# OAuth 2.0 should only occur over https.
		authorization_response = self.wsgi_app.last_request_uri.replace(
			'http', 'https')
...
class _RedirectWSGIApp(object):
	...
	def __call__(self, environ, start_response):
		...
		return [("<html lang='ja'><head><title>Authorization result</title><meta charset='utf-8'></head><body>"+self._success_message+"</body></html>").encode('utf-8')]
