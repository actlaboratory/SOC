...
class ViewCreator():
	...
	def combobox(self,text, selection, event=None, state=-1, style=wx.CB_READONLY, x=-1, sizerFlag=wx.ALL, proportion=0,textLayout=wx.DEFAULT):
		hStaticText,sizer=addDescriptionText(text,textLayout)
	...
	def spinCtrl(self,text, min=0, max=100, event=None, defaultValue=0, style=wx.SP_ARROW_KEYS, x=0, sizerFlag=wx.ALL, proportion=0,textLayout=wx.DEFAULT):
		hStaticText,sizer=addDescriptionText(text,textLayout)
	...
	def slider(self,text, min=0, max=100, event=None, defaultValue=0, style=0, x=0, sizerFlag=wx.ALL, proportion=0,textLayout=wx.DEFAULT):
		hStaticText,sizer=addDescriptionText(text,textLayout)
	...
	def addDescriptionText(self,text,textLayout):
		hStaticText=wx.StaticText(self.parent,wx.ID_ANY,label=text,name=text)
		self.SetFace(hStaticText)
		if type(self.sizer) in (wx.BoxSizer,wx.StaticBoxSizer) and textLayout!=self.sizer.GetOrientation():
