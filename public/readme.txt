...
6. 認証官僚ページが表示されますのでそのページを閉じてSOCのウインドウに戻ります。
７．認証が完了したという内容のダイアログが表示されるので閉じれば認証は完了です。
...
4. 変換に際しては、使用するOCRエンジンに関係なく、PC本体に一時的に画像ファイルを保存する場合があります。
	...
	6-7  設定画面について
メニューバーの設定➡設定画面を開くから設定を行うことができます。
OKボタンで設定を保存します。
キャンセルボタンで設定を保存せずに画面を閉じます。
１．一般タブ
スクリーンリーダー: 使用するスクリーンリーダーを選択します。
配色: 画面の配色を設定します。
起動時にアップデートを確認: 起動時のアップデート確認の有無を設定します。
アップデート確認時のタイムアウト秒数: アップデート確認時の通信がタイムアウトする時間を設定します。ここには数字のみを入れることができます。
２．OCR
一時ファイルの場所: OCR時に使用する一時ファイルの保存先を設定します。
認識結果の保存先: 認識結果のテキストファイルの保存先を設定します。ここで指定したディレクトリを選ぶとディレクトリが指定できるようになります。
...
６．このソフトはベータ版です。完成版でないことをご理解の上ご利用ください。
